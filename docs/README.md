...
The implementation currently has typed storage repositories and GraphQL DTOs for mutable entities, concrete event entities, GraphQL event interfaces with derived entity event relationships, `_meta`, current-state `block`/`subgraphError` compatibility arguments on entity and event roots, generated-style scalar filters for the main stored entity fields, `and/or` composition for `AccountFilter`, account relationship filters, and scalar-compatible `DomainFilter` predicates, concrete event-specific filters, shallow trailing-underscore relationship filters, operational CLI commands, and a confirmation-depth live indexing loop with parent-hash reorg detection. Reorg repair currently performs a coarse full indexed-state reset and canonical rebuild. The remaining API parity work is recursive relationship filters, broad `and/or` composition for registration/resolver/wrapped-domain/event filters, relationship ordering, event-interface-specific filters, and historical block/change-block query semantics; the remaining indexing hardening work is efficient common-ancestor rollback and differential validation.
