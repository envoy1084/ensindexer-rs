...
Last full verification: `cargo run -p cli -- schema-diff --output target/official-subgraph-schema.json && make check` passed after commit `f484c3e`.
...
- [ ] Add entity change logs or snapshots to support efficient rollback, `_change_block`, and historical reads.
